...
public class maxContingousArea {
        ...
        for(int i=0; i < m_row; i++)
        {
            for (int j=0; j< m_col; j++)
            {
                if(isVisited[i][j] == false)
                {
                    ...
                    mListMaxRegion.put(currentRegion, m_currentArea);
                }
            }
        }

        // Traverse throught stack to get highest region.
        mListMaxRegion.entrySet().forEach(entry -> {
    ...
    public static void main(String[] args)
    {
        ...
        int[][] input4 = {{1, 2, 1, 2, 1}, {1, 2, 1, 1, 1}, {1, 1, 2, 1, 1}, {1, 2, 2, 2, 1}}; // out of bound sample test

        System.out.print("Highest  max area for current input is = "+ CheckLargestRegion(input3));
    }
}